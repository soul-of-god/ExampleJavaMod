package lightglories.world.blocks.distribution;
...
import arc.util.*;
import arc.util.io.*;
import lightglories.LightGlories;
import lightglories.world.blocks.payloads.*;
...
public class Spinner extends Block {
    ...
    public void load() {
        ...
        laser = atlas.find(name + "-laser", "lightglories-spinner-laser");
        laserEnd = atlas.find(name + "-laser-end", "lightglories-spinner-laser-end");
    ...
    public class SpinnerBuild extends Building {
        ...
        public void read(Reads read, byte revision){
            ...
            if(spinning){
                ...
                if(spin >= spinTime) looped = true;
                offset = read.b();
                if(mobile) payload = LightGlories.mobileUtil.readPayload(read);
        ...
        public void write(Writes write){
            ...
            if(spinning){
                ...
                write.b(offset);
                if(mobile) LightGlories.mobileUtil.writePayload(payload, write);
        ...
        public void read(Reads read, byte revision){
            ...
            for(int i = 0; i < size; i++){
                BuildPayload pay = mobile ? LightGlories.mobileUtil.readPayload(read) : Payload.read(read);
                builds.add(pay);
            }
        ...
        public void write(Writes write){
            write.i(builds.size);
            builds.forEach(b -> {
                if(mobile) LightGlories.mobileUtil.writePayload(b, write);
